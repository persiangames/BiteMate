...
let EmailService = EmailService_1 = class EmailService {
    ...
    getTransporter() {
        ...
        const host = this.configService.get('messaging.email.smtp.host');
        const user = this.configService.get('messaging.email.smtp.user');
        const pass = this.configService.get('messaging.email.smtp.pass');
        if (!host || !user || !pass) {
            return null;
        }
        this.transporter = nodemailer.createTransport({
            host,
            port: this.configService.get('messaging.email.smtp.port', 587),
            secure: this.configService.get('messaging.email.smtp.secure', false),
            auth: { user, pass },
        });
        return this.transporter;
    }
    async sendOtpEmail(to, code, purpose) {
        const provider = this.configService.get('messaging.email.provider', 'console');
        const appName = this.configService.get('messaging.appName', 'BiteMate');
        const from = this.configService.get('messaging.email.from', 'noreply@bitemate.ir');
        const fromName = this.configService.get('messaging.email.fromName', 'BiteMate');
