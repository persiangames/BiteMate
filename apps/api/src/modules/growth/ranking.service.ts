...
export class RankingService {
  ...
  async getUserRankings(query: RankingsQueryDto): Promise<UserRankingsResponseDto> {
    ...
    const response: UserRankingsResponseDto = {
  ...
  async getRestaurantRankings(
    ...
    const response: RestaurantRankingsResponseDto = {
      city: query.city ?? null,
      updatedAt: new Date().toISOString(),
  ...
  async refreshUserRank(userId: string): Promise<number> {
  ...
  async refreshRestaurantRank(restaurantId: string): Promise<number> {
  ...
  async recordActivity(userId: string, points: number): Promise<void> {
  ...
  async recordRestaurantConversion(restaurantId: string): Promise<void> {
    await this.prisma.restaurant.update({
      where: { id: restaurantId },
  ...
  computeUserScore(params: {
    ...
    const reviewWeight =
      params.meetupReviewCount >= MIN_REVIEWS_FOR_FULL_WEIGHT
        ? 1
        : params.meetupReviewCount * 0.25;

    let score =
      params.successfulMeetups * 15 +
      params.meetupRating * Math.min(params.meetupReviewCount, 50) * 2 * reviewWeight +
      Math.min(params.activityPoints, 500) * 0.5 +
      (params.isPremium
        ? this.configService.get<number>('premium.priorityRankingBoost', 10)!
        : 0) -
      params.fraudPenalty;
  ...
  computeRestaurantScore(restaurant: Restaurant): number {
    const bayesianRating =
      (restaurant.averageRating * restaurant.reviewCount + 3.5 * 10) /
    ...
    const adBoost = restaurant.isSponsored
      ? this.configService.get<number>('premium.restaurantAdBoost', 15)!
      : 0;

    return (
      Math.round((reviewScore + visitScore + conversionScore + adBoost) * 100) / 100
    );
  }
  ...
  private async isEligibleForLeaderboard(userId: string, createdAt: Date): Promise<boolean> {
  ...
  private async computeFraudPenalty(userId: string): Promise<number> {
    const since = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
  ...
  private async countSuccessfulMeetups(userId: string): Promise<number> {
  ...
  private conversionRate(restaurant: Pick<Restaurant, 'visitCount' | 'conversionCount'>): number {
