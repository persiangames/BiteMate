...
export const locationConfig = registerAs('location', () => ({
  liveLocationTtlSeconds: parseInt(
    process.env.LIVE_LOCATION_TTL_SECONDS ?? '300',
    10,
  ),
  liveLocationUpdateIntervalSeconds: parseInt(
    process.env.LIVE_LOCATION_UPDATE_INTERVAL_SECONDS ?? '30',
    10,
  ),
...
export const intentConfig = registerAs('intent', () => ({
...
export const walletConfig = registerAs('wallet', () => ({
  encryptionKey:
    process.env.WALLET_ENCRYPTION_KEY ?? 'dev-only-wallet-encryption-key-32chars!!',
